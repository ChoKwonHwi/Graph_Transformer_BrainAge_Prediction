...
class Brain_image(image_dataset):
    ...
    def get_data(self):
        ...
        sub_dir = natsort.natsorted(list_dir)
...
class Brain_network(graph_dataset):
    ...
    def process(self):
        # define file paths
        #node_path = os.path.join('/nasdata4/kwonhwi/GraphTransformer/GraphTransformer_BrainAge/brain_network/ADNI_pro/AD/Fusion/node_feature')
        #adjacency_path = os.path.join('/nasdata4/kwonhwi/GraphTransformer/GraphTransformer_BrainAge/brain_network/ADNI_pro/AD/Fusion/adjacency_matrix')
        #age_path = os.path.join('/nasdata4/kwonhwi/GraphTransformer/GraphTransformer_BrainAge/brain_network/ADNI_pro/AD/Fusion/age')

        node_path = os.path.join('/nasdata4/kwonhwi/GraphTransformer/GraphTransformer_BrainAge/brain_network/project/Fusion/node_feature')
        adjacency_path = os.path.join('/nasdata4/kwonhwi/GraphTransformer/GraphTransformer_BrainAge/brain_network/project/Fusion/adjacency_matrix')
        age_path = os.path.join('/nasdata4/kwonhwi/GraphTransformer/GraphTransformer_BrainAge/brain_network/project/Fusion/age')

        sub_dir = os.listdir(node_path)
        #sub_dir.sort(key=lambda x: int(x[:-4]))
        sub_dir = natsort.natsorted(sub_dir)
        # load data and label
        index = 0
        for name in sub_dir:
