...
parser.add_argument('--output_csv_path', type=str, default='/nasdata4/kwonhwi/GraphTransformer/GraphTransformer_BrainAge/ugugugugugugug/GT_PE_loss.csv')
...
def main():
    ...
    net = GraphNet_PE.GraphNet(input_dim=args.input_dim)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            # print(f" - x: {data.x.shape}, Edge Index: {data.edge_index.shape}, Batch: {data.batch.shape}")
            # print(f" - Device: {data.x.device}")
            # print(f" - Device: {data.edge_index.device}")

            # print(f"Device for data.x: {data.x.device}")
            # print(f"Device for model: {[param.device for param in net.parameters()]}")
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(net.state_dict(), os.path.join(model_path, 'best_model_PE.pth'))
