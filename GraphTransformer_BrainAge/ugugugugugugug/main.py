...
def main():
    ...
    data_path = '../brain_network/ADNI_pro/AD'
    ...
    # get trained model
    save_model = torch.load(os.path.join(model_path, 'best_model_PE.pth')) # 여기 경로 수정
    #save_model = torch.load(os.path.join(model_path, 'best_model_GCN2_GT_onlymri.pth'))
    ...
    print(true_age)
    results = pd.DataFrame({'True Age': true_age, 'Predicted Age': pred_age})
    results.to_csv('./true_vs_pred_age.csv', index=False)
    print("Results saved to 'true_vs_pred_age.csv'.")
    ...
    # 추세선 (회귀선) 추가
    z = np.polyfit(true_age, pred_age, 1)  # 1차 다항식 (선형 회귀)
    p = np.poly1d(z)
